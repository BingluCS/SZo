...
"""Python wrapper for SZo Config class."""
...
cdef class szoConfig:
    """
    Configuration class for SZo compression.
    
    Enum classes: szoErrorBoundMode, szoAlgorithm, szoInterpAlgorithm
    
    Parameters: Dimension sizes as individual ints, tuple, or list
        szoConfig(data.shape), szoConfig(100, 200, 300), etc.
    """
    ...
    def errorBoundMode(self):
        """Get/set error bound mode. Use szoErrorBoundMode enum."""
    ...
    def errorBoundMode(self, value):
        if hasattr(value, 'value'):  # Is an enum
            ...
            raise TypeError(f"Use szoErrorBoundMode enum, got {type(value).__name__}")
    
    @property
    def cmprAlgo(self):
        """Get/set compression algorithm. Use szoAlgorithm enum."""
    ...
    def cmprAlgo(self, value):
        if hasattr(value, 'value'):  # Is an enum
            ...
            raise TypeError(f"Use szoAlgorithm enum, got {type(value).__name__}")
    
    @property
    def interpAlgo(self):
        """Get/set interpolation algorithm. Use szoInterpAlgorithm enum."""
    ...
    def interpAlgo(self, value):
        if hasattr(value, 'value'):  # Is an enum
            ...
            raise TypeError(f"Use szoInterpAlgorithm enum, got {type(value).__name__}")
    ...
    def __repr__(self):
        return f"szoConfig(dims={self.dims}, num_elements={self.num_elements})"
