#!/usr/bin/env python3
"""
Convert SZo parameters to HDF5 cd_values
...
class SZo:
    def __init__(self, absolute=None, relative=None, psnr=None, l2norm=None):
        ...
        serialized.extend(struct.pack('<I', 0))  # placeholder for SZo_MAGIC_NUMBER
        serialized.extend(struct.pack('<I', 0))  # placeholder for SZo_DATA_VER
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Convert SZo error bounds to HDF5 cd_values',
        epilog="""
Examples:
  %(prog)s --abs 1e-3
  %(prog)s --rel 0.001
  ...
  h5repack $(%(prog)s --abs 1e-3) input.h5 output.h5
    ...
    try:
        if args.absolute is not None:
            config = SZo(absolute=args.absolute)
        elif args.relative is not None:
            config = SZo(relative=args.relative)
        elif args.psnr is not None:
            config = SZo(psnr=args.psnr)
        else:
            config = SZo(l2norm=args.l2norm)
