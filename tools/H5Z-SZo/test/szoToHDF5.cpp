...
template <typename T>
void process_data(const SZo::Config &conf, const char *oriFilePath, int dataEndianType, hid_t fid, hid_t sid,
                  hid_t cpid, const char *datasetName, hid_t h5TypeLE, hid_t h5TypeBE) {
    T *data = new T[conf.num];
    SZo::readfile(oriFilePath, conf.num, data);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        printf("Test case: szoToHDF5 [dataType] [srcFilePath] [dimension sizes...]\n");
        printf("Example1 : szoToHDF5 -f testdata/x86/testfloat_8_8_128.dat 8 8 128\n");
        printf("Example 2: szoToHDF5 -i32 testdata/x86/testint32_8x8x8.dat 8 8 8\n");
        exit(0);
    }
    ...
    if (it != dataTypeMap.end()) {
        ...
        std::cerr << "Error: unknown data type in szoToHDF5.c!\n";
        return 0;
    }
    ...
    snprintf(outputFilePath, 640, "%s.szo.h5", oriFilePath);
    ...
    set_SZo_conf_to_H5(cpid, conf);

    if (0 > H5Zregister(H5PLget_plugin_info())) {
        printf("Error in H5Zregister");
        exit(0);
    }
    if (H5Zfilter_avail(H5Z_FILTER_SZo)) {
        unsigned filter_config;
        if (H5Zget_filter_info(H5Z_FILTER_SZo, &filter_config) & H5Z_FILTER_CONFIG_ENCODE_ENABLED)
    ...
    {  // This is an example to get/set SZ configuration from HDF5 file
        SZo::Config conf1;
        get_SZo_conf_from_H5(cpid, conf1);
        //        conf1.absErrorBound = 1;
        set_SZo_conf_to_H5(cpid, conf1);
    }
    ...
    if (H5Zunregister(H5Z_FILTER_SZo) < 0) return -1;
    H5close();
    return 0;
}
