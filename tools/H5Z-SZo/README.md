...
# H5Z-SZo Filter Integration

The H5Z-SZo filter integrates the SZ3 compression library with HDF5, providing an efficient way to compress and decompress data within HDF5 files.
...
Compile and install SZ3 with the H5Z-SZo filter enabled:
```bash
add -DBUILD_H5Z_FILTER=true to cmake command to enable H5Z-SZo filter in SZ3
```

### Step 2: Configure Environment
Add the directory containing the H5Z-SZo library (`libhdf5szo.so` or `libhdf5szo.dylib`) to your `HDF5_PLUGIN_PATH` and `LD_LIBRARY_PATH` environment variables:
```bash
export HDF5_PLUGIN_PATH=${HDF5_PLUGIN_PATH}:<PATH_TO_YOUR_H5Z-SZo_LIB>/
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$HDF5_HOME/lib:$<PATH_TO_YOUR_H5Z-SZo_LIB>
```


## Use H5Z-SZo in HDF5 Executables
...
See examples `szoToHDF5.cpp` and `HDF5ToSzo.cpp` for how to use the H5Z-SZo filter in your C/C++ projects.
