...
// filter definition
const H5Z_class2_t H5Z_SZo[1] = {{
    H5Z_CLASS_T_VERS,                                       /* H5Z_class_t version */
    H5Z_FILTER_SZo,                                         /* Filter id number */
    1,                                                      /* encoder_present flag (set to true) */
    1,                                                      /* decoder_present flag (set to true) */
    "SZo compressor/decompressor for floating-point data.", /* Filter name for debugging */
    NULL,                                                   /* The "can apply" callback */
    H5Z_szo_set_local,                                      /* The "set local" callback */
    static_cast<H5Z_func_t>(H5Z_filter_szo),                /* The actual filter function */
}};

HDF5SZo_EXPORT H5PL_type_t H5PLget_plugin_type(void) { return H5PL_TYPE_FILTER; }

HDF5SZo_EXPORT const void* H5PLget_plugin_info(void) { return H5Z_SZo; }

herr_t set_SZo_conf_to_H5(const hid_t propertyList, SZo::Config& conf) {
    static char const* _funcname_ = "set_SZo_conf_to_H5";
    ...
    auto szfilter = H5Pget_filter_by_id(propertyList, H5Z_FILTER_SZo, H5Z_FLAG_MANDATORY, NULL, NULL, 0, NULL,
    ...
    if (0 > szfilter) {
        ...
        if (0 > H5Pset_filter(propertyList, H5Z_FILTER_SZo, H5Z_FLAG_MANDATORY, cd_nelmts, cd_values.data())) {
            H5Z_SZ_PUSH_AND_GOTO(H5E_PLINE, H5E_BADVALUE, 0, "failed to modify cd_values");
        }
    } else {
        // filter already set, update filter
        if (0 > H5Pmodify_filter(propertyList, H5Z_FILTER_SZo, H5Z_FLAG_MANDATORY, cd_nelmts, cd_values.data()))
            H5Z_SZ_PUSH_AND_GOTO(H5E_PLINE, H5E_BADVALUE, 0, "failed to modify cd_values");
    }
...
herr_t get_SZo_conf_from_H5(const hid_t propertyList, SZo::Config& conf) {
    static char const* _funcname_ = "get_SZo_conf_from_H5";
    ...
    if (0 > H5Pget_filter_by_id(propertyList, H5Z_FILTER_SZo, H5Z_FLAG_MANDATORY, &cd_nelmts, cd_values.data(), 0, NULL,
                                NULL))
...
static herr_t H5Z_szo_set_local(hid_t dcpl_id, hid_t type_id, hid_t chunk_space_id) {
    printf("start H5Z_szo_set_local\n");

    // printf("start in H5Z_szo_set_local, dcpl_id = %d\n", dcpl_id);
    static char const* _funcname_ = "H5Z_szo_set_local";

    // herr_t ret = H5Zregister(H5Z_SZo);

    SZo::Config conf;
    get_SZo_conf_from_H5(dcpl_id, conf);
...
template <typename T>
void process_data(SZo::Config& conf, void** buf, size_t* buf_size, size_t nbytes, bool is_decompress) {
    if (is_decompress) {
