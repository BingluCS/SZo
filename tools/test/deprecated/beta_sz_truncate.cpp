...
float SZ_compress(std::unique_ptr<T[]> const &data,
                  const SZo::Config &conf, Lossless lossless) {
    ...
    auto sz = SZo::make_sz_truncate_compressor<T, N>(conf, lossless, byteLen);
...
float SZ_compress_parse_args(int argc, char **argv, int argp, std::unique_ptr<T[]> &data, float eb,
    ...
    if (conf.lossless > 0) {
        return SZ_compress<T, N>(data, conf, SZo::Lossless_zstd(conf.lossless));
    } else {
        return SZ_compress<T, N>(data, conf, SZo::Lossless_bypass());
    }
}
