...
#ifndef SZo_LOSSLESS_ZSTD_HPP
#define SZo_LOSSLESS_ZSTD_HPP
...
class Lossless_zstd : public concepts::LosslessInterface {
   public:
    ...
    size_t compress(const uchar *src, size_t srcLen, uchar *dst, size_t dstCap) override {
        ...
        if (std::getenv("SZO_NOZSTD")) {                 // benchmark: skip zstd, store raw
            if (dstCap < srcLen) throw std::length_error(SZo_ERROR_COMP_BUFFER_NOT_LARGE_ENOUGH);
            memcpy(dst, src, srcLen);
            return srcLen + sizeof(size_t);
        }
        if (dstCap < ZSTD_compressBound(srcLen)) {
            throw std::length_error(SZo_ERROR_COMP_BUFFER_NOT_LARGE_ENOUGH);
        }
    ...
    size_t decompress(const uchar *src, const size_t srcLen, uchar *&dst, size_t &dstLen) override {
        ...
        if (std::getenv("SZO_NOZSTD")) {                 // benchmark: data stored raw
