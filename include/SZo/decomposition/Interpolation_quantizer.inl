...
namespace SZo {
#ifdef __AVX2__
    // quant codes stored as uint16 (radius<=32768 => values in [0,65535]) to halve quant_inds traffic.
#ifndef SZo_QUANT16_HELPERS
#define SZo_QUANT16_HELPERS
    static ALWAYS_INLINE void store_quant8(int16_t* p, __m256i v) {
        const __m128i hi = _mm256_extracti128_si256(v, 1);
        const __m128i packed = _mm_packs_epi32(_mm256_castsi256_si128(v), hi);  // [v0..v7], unsigned saturate
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear_and_quantize_1D_line(
        T *data, const size_t &n, size_t& stride, size_t& cur_ij_offset, QuantizeFunc &&quantize_func) {
        if (n <= 1) {
            return;
        }
        auto odd_len = n / 2;
        auto even_len = n - odd_len;
        size_t i = 0;
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + step < odd_len; i += step) {
                ...
                for (size_t j = 0; j < step; ++j) {
                    size_t even = (i + j) << 1;
                    a[j] = data[cur_ij_offset + even * stride];
                    b[j] = data[cur_ij_offset + (even + 2) * stride];
                }
                ...
                if constexpr (CompMode == COMPMODE::COMP) {
                    ...
                    #pragma unroll
                    for (size_t j = 0; j < step; ++j) {
                        ori[j] = data[cur_ij_offset + (start + (j << 1)) * stride];
                    }
                    __m256 ori_avx = _mm256_loadu_ps(ori);
                    __m256 quant_avx = _mm256_sub_ps(ori_avx, sum);
                    float tmp[8];
                    ...
                    if constexpr (!SkipOverwrite) {
                        #pragma unroll
                        for (size_t j = 0; j < step; ++j) {
                            data[cur_ij_offset + (start + (j << 1)) * stride] = tmp[j];
                        }
                    }
                    ...
                    while (esc) {
                        ...
                        if constexpr (!SkipOverwrite) {
                            data[cur_ij_offset + (start + (k << 1)) * stride] = ori[k];
                        }
                        quantizer.force_save_unpred(ori[k]);
                        esc &= esc - 1;
                    }
                    store_quant8(quant_inds + quant_index, quant_avx_i);
                } else {
                    __m256i quant_avx_i = load_quant8(quant_inds + quant_index);
                    __m256d decompressed_low = _mm256_cvtepi32_pd(_mm256_castsi256_si128(quant_avx_i));
                    decompressed_low = _mm256_fmadd_pd(decompressed_low, ebx2_avx,
                    ...
                    decompressed_high = _mm256_fmadd_pd(decompressed_high, ebx2_avx,
                    ...
                    while (esc) {
                        int k = __builtin_ctz(esc);
                        data[cur_ij_offset + (start + (k << 1)) * stride] = quantizer.recover_unpred();
                        esc &= esc - 1;
                    }
                }
                quant_index += step;
            ...
            for (; i + step < odd_len; i += step) {
                ...
                if constexpr (CompMode == COMPMODE::COMP) {
                    ...
                    __m256d quant_avx = _mm256_sub_pd(ori_avx, sum);
                    T tmp[4];
                    ...
                    store_quant4(quant_inds + quant_index, quant_avx_i);
                } else {
                    __m128i quant_avx_i = load_quant4(quant_inds + quant_index);
                    __m256d decompressed = _mm256_fmadd_pd(_mm256_cvtepi32_pd(quant_avx_i), ebx2_avx, sum);
                    T tmp[4];
        ...
        for (; i + 1 < odd_len; ++i) {
            size_t start = ((i << 1) + 1) * stride;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start, data[cur_ij_offset + start],
                          interp_linear(data[cur_ij_offset + (i << 1) * stride],
                                        data[cur_ij_offset + ((i << 1) + 2) * stride]));
        }
        T pred_edge = (n < 3) ? data[cur_ij_offset + ((even_len - 1) << 1) * stride]
                              : interp_linear1(data[cur_ij_offset + ((even_len - 2) << 1) * stride],
                                               data[cur_ij_offset + ((even_len - 1) << 1) * stride]);
        int last = 2 * odd_len - 1;
        quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + last * stride, data[cur_ij_offset + last * stride], pred_edge);
    }

    template <TUNING Tuning, class T, uint N, class Quantizer>
    template <COMPMODE CompMode, bool SkipOverwrite, class QuantizeFunc>
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_cubic_and_quantize_1D_line(
        T *data, const size_t &n, size_t& stride, size_t& cur_ij_offset, QuantizeFunc &&quantize_func) {
        if (n <= 1) {
            return;
        }
        ...
        if (even_len < 2) {
            ...
            pred_first = interp_linear(data[cur_ij_offset], data[cur_ij_offset + 2 * stride]);
        } else {
            pred_first = interp_quad_1(data[cur_ij_offset], data[cur_ij_offset + 2 * stride],
                                       data[cur_ij_offset + 4 * stride]);
        }
        quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + stride, data[cur_ij_offset + stride], pred_first);
...
#pragma unroll
                for (size_t j = 0; j < step; ++j) {
                    ...
                    a[j] = data[cur_ij_offset + even * stride];
                    b[j] = data[cur_ij_offset + (even + 2) * stride];
                    c[j] = data[cur_ij_offset + (even + 4) * stride];
                    d[j] = data[cur_ij_offset + (even + 6) * stride];
                }
...
#pragma unroll
        ...
        for (; i + 3 < even_len; ++i) {
            size_t start = ((i << 1) + 3) * stride;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start, data[cur_ij_offset + start],
                          interp_cubic(data[cur_ij_offset + (i << 1) * stride],
                                       data[cur_ij_offset + ((i << 1) + 2) * stride],
                                       data[cur_ij_offset + ((i << 1) + 4) * stride],
                                       data[cur_ij_offset + ((i << 1) + 6) * stride]));
        }
        if (odd_len > 1) {
            if (odd_len < even_len) {
                T edge_pred = interp_quad_2(data[cur_ij_offset + ((even_len - 3) << 1) * stride],
                                            data[cur_ij_offset + ((even_len - 2) << 1) * stride],
                                            data[cur_ij_offset + ((even_len - 1) << 1) * stride]);
                int last = 2 * odd_len - 1;
                quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + last * stride, data[cur_ij_offset + last * stride], edge_pred);
            } else {
                if (odd_len > 2) {
                    T edge_pred = interp_quad_2(data[cur_ij_offset + ((even_len - 3) << 1) * stride],
                                                data[cur_ij_offset + ((even_len - 2) << 1) * stride],
                ...
                T edge_pred = interp_linear1(data[cur_ij_offset + ((even_len - 2) << 1) * stride],
                                             data[cur_ij_offset + ((even_len - 1) << 1) * stride]);
                int last = 2 * odd_len - 1;
                quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + last * stride, data[cur_ij_offset + last * stride], edge_pred);
            }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear_and_quantize_1D(const T * buf, const size_t &len, T* data,
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + step < odd_len; i += step) {
                ...
                if constexpr (CompMode == COMPMODE::COMP) {
                    ...
                    while (esc) {
                        int k = __builtin_ctz(esc);
                        data[(start + (k << 1)) * offset] = quantizer.recover_unpred();
                        esc &= esc - 1;
                    }
                    quant_index += step;
                }
            ...
            for (; i + 1 < odd_len; ++i) {
                ...
                quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start, data[start], interp_linear(buf[i], buf[i + 1]));
            }
        }
        else if constexpr (std::is_same_v<T, double>) {
            static constexpr size_t step = AVX_256_parallelism;
        ...
        int last = 2 * odd_len - 1;
        quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + last * offset , data[last * offset], pred_edge);
    }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_cubic_and_quantize_1D(const T * buf, const size_t &len, T* data,
       ...
       // assert(len <= max_dim);
        ...
        quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + offset , data[offset], pred_first);
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + 3 < even_len; ++i) {
                ...
                quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start, data[start],
                              interp_cubic(buf[i], buf[i + 1], buf[i + 2], buf[i + 3]));
            }
        }
        else if constexpr (std::is_same_v<T, double>) {
            static constexpr size_t step = AVX_256_parallelism;
        ...
        if(odd_len > 1){
            if(odd_len < even_len){//the only boundary is p[len- 1]
                ...
                int last = 2 * odd_len - 1;
                quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + last * offset, data[last * offset], edge_pred);
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear_and_quantize(const T * a, const T* b, size_t &len, T* data,
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + step <= len; i += step) {
                __m256 sum = _mm256_mul_ps(_mm256_add_ps(_mm256_loadu_ps(a + i), _mm256_loadu_ps(b + i)), factor);
                quantize_float<CompMode, step, true, SkipOverwrite>(sum, i, data, offset, len);
            }
            if (i < len) {
                __m256 sum = _mm256_mul_ps(_mm256_add_ps(_mm256_loadu_ps(a + i), _mm256_loadu_ps(b + i)), factor);
                quantize_float<CompMode, step, false, SkipOverwrite>(sum, i, data, offset, len);
            }
        }
        else if constexpr (std::is_same_v<T, double>) {
            static constexpr size_t step = AVX_256_parallelism;
            ...
            for (; i + step <= len; i += step) {
                ...
                // size_t start = i;
                quantize_double<CompMode, step, true, SkipOverwrite>(sum, i, data, offset, len);
            }
            if (i < len) {
                __m256d va = _mm256_loadu_pd(a + i);
                __m256d vb = _mm256_loadu_pd(b + i);
                ...
                // _mm256_storeu_pd(p + i, sum);
                // size_t start = i;
                quantize_double<CompMode, step, false, SkipOverwrite>(sum, i, data, offset, len);
            }
        }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::quantize_float (__m256& sum, size_t& start, T*& data, size_t& offset, size_t& len) {
        if constexpr (CompMode == COMPMODE::COMP) {
            ...
            if constexpr (FullOnly) {
                // caller guarantees a full chunk: read straight, skip the per-lane tail routing
            ...
            if constexpr (FullOnly) {
                ...
                _mm256_storeu_ps(tmp, out);
                #pragma unroll
                for ( ; processed < step && start + processed < len; ++processed)
                    if constexpr (!SkipOverwrite) data[(start + processed) * offset] = tmp[processed];
            ...
            if constexpr (FullOnly) {
                ...
                if (offset == 1) {
                    ...
                    while (esc) { int k = __builtin_ctz(esc); data[start + k] = quantizer.recover_unpred(); esc &= esc - 1; }
                } else {
                    float tmp[8];
                    _mm256_storeu_ps(tmp, decompressed);
                    #pragma unroll
                    for (size_t j = 0; j < step; ++j) data[(start + j) * offset] = tmp[j];
                    while (esc) { int k = __builtin_ctz(esc); data[(start + k) * offset] = quantizer.recover_unpred(); esc &= esc - 1; }
                }
            } else {
                float tmp[8];
                _mm256_storeu_ps(tmp, decompressed);
                #pragma unroll
                for ( ; processed < step && start + processed < len; ++processed)
                    data[(start + processed) * offset] = tmp[processed];
                unsigned esc = esc_all & ((1u << processed) - 1);
                while (esc) {
                    int k = __builtin_ctz(esc);
                    data[(start + k) * offset] = quantizer.recover_unpred();
                    esc &= esc - 1;
                }
            }
            quant_index += processed;
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear_and_quantize_1D_line(
        ...
        auto odd_len = n / 2;
        auto even_len = n - odd_len;
        // Rebase onto the odd (predicted) positions so the standard strided
        // quantize_* helper handles gather/scatter/escape exactly as the batch path.
        T *odd_data = data + cur_ij_offset + stride;
        size_t odd_stride = stride << 1;
        size_t i = 0;
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + step < odd_len; i += step) {
                ...
                size_t start = i;
                quantize_float<CompMode, false, SkipOverwrite>(sum, start, odd_data, odd_stride, odd_len, step, pg, pg64);
            }
        } else if constexpr (std::is_same_v<T, double>) {
            static const size_t step = SVE2_parallelism;
            svbool_t pg64 = svptrue_b64();
            for (; i + step < odd_len; i += step) {
                ...
                size_t start = i;
                quantize_double<CompMode, false, SkipOverwrite>(sum, start, odd_data, odd_stride, odd_len, step, pg64);
            }
        }
        for (; i + 1 < odd_len; ++i) {
            size_t start = ((i << 1) + 1) * stride;
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + 1  < even_len; i += step) { // 3 is not AVX_256_parallelism - 1 !!
                ...
                if constexpr (CompMode == COMPMODE::COMP) {
                    ...
                    svst1h_s32(pg, quant_inds + quant_index, quant_sve_i);
                    quant_index += j;
                }
                else if constexpr (CompMode == COMPMODE::DECOMP) { // decomp
                    svint32_t quant_sve_i = svld1sh_s32(pg, quant_inds + quant_index);
                    int quant_vals[step];
                    svst1(pg, quant_vals, quant_sve_i);
                    ...
                    decompressed_even_f64 = svmla_f64_x(pg64, svcvt_f64_f32_x(pg64, sum),
                                                decompressed_even_f64, svdup_f64(real_ebx2));
                    decompressed_odd_f64  = svmla_f64_x(pg64, svcvtlt_f64_f32_x(pg64, sum),
                                                decompressed_odd_f64, svdup_f64(real_ebx2));
            ...
            for (; i + 1 < even_len; i += step) { // 3 is not AVX_256_parallelism - 1 !!
                ...
                if constexpr (CompMode == COMPMODE::COMP) {
                    ...
                    svst1h_s64(pg64, quant_inds + quant_index, quant_sve_i);
                    quant_index += j;
                }
                else if constexpr (CompMode == COMPMODE::DECOMP) { // decomp
                    svint64_t quant_sve_i = svld1sh_s64(pg64, quant_inds + quant_index);
                    int quant_vals[step];
                    svst1w_s64(pg64, quant_vals, quant_sve_i);
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear_and_quantize(const T * a, const T* b, size_t &len, T* data,
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + step <= len; i += step) {
                ...
                quantize_float<CompMode, true, SkipOverwrite>(sum, i, data, offset, len, step, pg, pg64);
            }
            if (i < len) {
                svfloat32_t va = svld1(pg, &a[i]);
                svfloat32_t vb = svld1(pg, &b[i]);
                ...
                quantize_float<CompMode, false, SkipOverwrite>(sum, i, data, offset, len, step, pg, pg64);
            }
        }
        else if constexpr (std::is_same_v<T, double>) {
            static const size_t step = SVE2_parallelism;
            ...
            for (; i + step <= len; i += step) {
                ...
                svfloat64_t sum = svadd_f64_x(pg64, va, vb);
                sum = svmul_n_f64_x(pg64, sum, 0.5);
                quantize_double<CompMode, true, SkipOverwrite>(sum, i, data, offset, len, step, pg64);
            }
            if (i < len) {
                svfloat64_t va = svld1(pg64, &a[i]);
                svfloat64_t vb = svld1(pg64, &b[i]);
                ...
                svfloat64_t sum = svadd_f64_x(pg64, va, vb);
                sum = svmul_n_f64_x(pg64, sum, 0.5);
                quantize_double<CompMode, false, SkipOverwrite>(sum, i, data, offset, len, step, pg64);
            }
        }
    }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::quantize_float (svfloat32_t& sum, size_t& start, T*& data, size_t& offset,
        ...
        const size_t processed = FullOnly ? step : std::min(step, len - start);
        svbool_t active_pg = pg;
        svbool_t low_pg64 = pg64;
        svbool_t high_pg64 = pg64;
        if constexpr (!FullOnly) {
            active_pg = svwhilelt_b32(static_cast<uint64_t>(0), static_cast<uint64_t>(processed));
            low_pg64 = svwhilelt_b64(
                static_cast<uint64_t>(0), static_cast<uint64_t>((processed + 1) >> 1));
            high_pg64 = svwhilelt_b64(
                static_cast<uint64_t>(0), static_cast<uint64_t>(processed >> 1));
        }

        if constexpr (CompMode == COMPMODE::COMP) {
            svfloat32_t ori_sve;
            if (offset == 1) {
                ori_sve = svld1_f32(active_pg, data + start);
            } else {
                const svuint32_t indices = svmul_n_u32_x(
                    active_pg, svindex_u32(static_cast<uint32_t>(start), 1),
                    static_cast<uint32_t>(offset));
                ori_sve = svld1_gather_u32index_f32(active_pg, data, indices);
            }
            svfloat32_t quant_sve = svsub_f32_x(active_pg, ori_sve, sum);

            svfloat64_t quant_even_f64 = svcvt_f64_f32_x(low_pg64, quant_sve);
            svfloat64_t quant_odd_f64 = svcvtlt_f64_f32_x(high_pg64, quant_sve);
            quant_even_f64 = svrintn_f64_x(
                low_pg64, svmul_n_f64_x(low_pg64, quant_even_f64, real_ebx2_r));
            quant_odd_f64 = svrintn_f64_x(
                high_pg64, svmul_n_f64_x(high_pg64, quant_odd_f64, real_ebx2_r));

            svbool_t pg_gt_neg = svcmpgt_n_f64(low_pg64, quant_even_f64, -radius);
            svbool_t pg_lt_pos = svcmplt_n_f64(low_pg64, quant_even_f64, radius);
            svbool_t pg_in_range = svand_b_z(low_pg64, pg_gt_neg, pg_lt_pos);
            quant_even_f64 = svsel_f64(pg_in_range, quant_even_f64, svdup_n_f64(0.0));

            svbool_t pg_gt_neg_o = svcmpgt_n_f64(high_pg64, quant_odd_f64, -radius);
            svbool_t pg_lt_pos_o = svcmplt_n_f64(high_pg64, quant_odd_f64, radius);
            svbool_t pg_in_range_o = svand_b_z(high_pg64, pg_gt_neg_o, pg_lt_pos_o);
            quant_odd_f64 = svsel_f64(pg_in_range_o, quant_odd_f64, svdup_n_f64(0.0));

            svfloat64_t decompressed_even_f64 = svmla_n_f64_x(
                low_pg64, svcvt_f64_f32_x(low_pg64, sum), quant_even_f64, real_ebx2);
            svfloat64_t decompressed_odd_f64 = svmla_n_f64_x(
                high_pg64, svcvtlt_f64_f32_x(high_pg64, sum), quant_odd_f64, real_ebx2);

            svfloat32_t decompressed = svcvt_f32_f64_x(low_pg64, decompressed_even_f64);
            decompressed = svcvtnt_f32_f64_x(decompressed, high_pg64, decompressed_odd_f64);

            quant_sve = svcvt_f32_f64_x(low_pg64, quant_even_f64);
            quant_sve = svcvtnt_f32_f64_x(quant_sve, high_pg64, quant_odd_f64);

            const svfloat32_t err_dequan = svsub_f32_x(active_pg, decompressed, ori_sve);
            pg_in_range = svand_b_z(
                active_pg, svcmpge_n_f32(active_pg, err_dequan, -real_eb),
                svcmple_n_f32(active_pg, err_dequan, real_eb));
            quant_sve = svsel_f32(pg_in_range, quant_sve, svdup_n_f32(-static_cast<float>(radius)));

            const svint32_t quant_sve_i = svcvt_s32_f32_z(active_pg, quant_sve);
            const svbool_t esc_pred = svcmpeq_n_s32(active_pg, quant_sve_i, -32768);
            if constexpr (!SkipOverwrite) {
                const svfloat32_t stored = svsel_f32(esc_pred, ori_sve, decompressed);
                if (offset == 1) {
                    svst1_f32(active_pg, data + start, stored);
                } else {
                    const svuint32_t indices = svmul_n_u32_x(
                        active_pg, svindex_u32(static_cast<uint32_t>(start), 1),
                        static_cast<uint32_t>(offset));
                    svst1_scatter_u32index_f32(active_pg, data, indices, stored);
                }
            }
            if (svptest_any(active_pg, esc_pred)) {
                T ori[step];
                int quant_vals[step];
                svst1_f32(active_pg, ori, ori_sve);
                svst1_s32(active_pg, quant_vals, quant_sve_i);
            ...
            svst1h_s32(active_pg, quant_inds + quant_index, quant_sve_i);
            quant_index += processed;
        }
        else if constexpr (CompMode == COMPMODE::DECOMP) {
            const svint32_t quant_sve_i =
                svld1sh_s32(active_pg, quant_inds + quant_index);
            svfloat64_t decompressed_even_f64 =
                svcvt_f64_s32_x(low_pg64, quant_sve_i);
            svfloat64_t decompressed_odd_f64 = svcvtlt_f64_f32_x(
                high_pg64, svcvt_f32_s32_x(active_pg, quant_sve_i));
            decompressed_even_f64 = svmla_n_f64_x(
                low_pg64, svcvt_f64_f32_x(low_pg64, sum),
                decompressed_even_f64, real_ebx2);
            decompressed_odd_f64 = svmla_n_f64_x(
                high_pg64, svcvtlt_f64_f32_x(high_pg64, sum),
                decompressed_odd_f64, real_ebx2);
            svfloat32_t decompressed =
                svcvt_f32_f64_x(low_pg64, decompressed_even_f64);
            decompressed =
                svcvtnt_f32_f64_x(decompressed, high_pg64, decompressed_odd_f64);

            if (offset == 1) {
                svst1_f32(active_pg, data + start, decompressed);
            } else {
                const svuint32_t indices = svmul_n_u32_x(
                    active_pg, svindex_u32(static_cast<uint32_t>(start), 1),
                    static_cast<uint32_t>(offset));
                svst1_scatter_u32index_f32(active_pg, data, indices, decompressed);
            }
            const svbool_t esc_pred =
                svcmpeq_n_s32(active_pg, quant_sve_i, -32768);
            if (svptest_any(active_pg, esc_pred)) {
                int quant_vals[step];
                svst1_s32(active_pg, quant_vals, quant_sve_i);
                for (size_t k = 0; k < processed; ++k) {
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::quantize_double (svfloat64_t& sum, size_t& start, T*& data, size_t& offset,
        ...
        const size_t processed = FullOnly ? step : std::min(step, len - start);
        svbool_t active_pg64 = pg64;
        if constexpr (!FullOnly) {
            active_pg64 =
                svwhilelt_b64(static_cast<uint64_t>(0), static_cast<uint64_t>(processed));
        }

        if constexpr (CompMode == COMPMODE::COMP) {
            svfloat64_t ori_sve;
            if (offset == 1) {
                ori_sve = svld1_f64(active_pg64, data + start);
            } else {
                const svuint64_t indices = svmul_n_u64_x(
                    active_pg64, svindex_u64(static_cast<uint64_t>(start), 1),
                    static_cast<uint64_t>(offset));
                ori_sve = svld1_gather_u64index_f64(active_pg64, data, indices);
            }
            svfloat64_t quant_sve = svsub_f64_x(active_pg64, ori_sve, sum);
            quant_sve = svrintn_f64_x(
                active_pg64,
                svmul_n_f64_x(active_pg64, quant_sve, real_ebx2_r));

            svbool_t pg_gt_neg = svcmpgt_n_f64(active_pg64, quant_sve, -radius);
            svbool_t pg_lt_pos = svcmplt_n_f64(active_pg64, quant_sve, radius);
            svbool_t pg_in_range = svand_b_z(active_pg64, pg_gt_neg, pg_lt_pos);
            quant_sve = svsel_f64(pg_in_range, quant_sve, svdup_n_f64(0.0));

            const svfloat64_t decompressed =
                svmla_n_f64_x(active_pg64, sum, quant_sve, real_ebx2);
            const svfloat64_t err_dequan =
                svsub_f64_x(active_pg64, decompressed, ori_sve);
            pg_in_range = svand_b_z(
                active_pg64, svcmpge_n_f64(active_pg64, err_dequan, -real_eb),
                svcmple_n_f64(active_pg64, err_dequan, real_eb));
            quant_sve = svsel_f64(pg_in_range, quant_sve, svdup_n_f64(-static_cast<double>(radius)));

            const svint64_t quant_sve_i = svcvt_s64_f64_x(active_pg64, quant_sve);
            const svbool_t esc_pred =
                svcmpeq_n_s64(active_pg64, quant_sve_i, -32768);
            if constexpr (!SkipOverwrite) {
                const svfloat64_t stored = svsel_f64(esc_pred, ori_sve, decompressed);
                if (offset == 1) {
                    svst1_f64(active_pg64, data + start, stored);
                } else {
                    const svuint64_t indices = svmul_n_u64_x(
                        active_pg64, svindex_u64(static_cast<uint64_t>(start), 1),
                        static_cast<uint64_t>(offset));
                    svst1_scatter_u64index_f64(active_pg64, data, indices, stored);
                }
            }
            if (svptest_any(active_pg64, esc_pred)) {
                T ori[step];
                int quant_vals[step];
                svst1_f64(active_pg64, ori, ori_sve);
                svst1w_s64(active_pg64, quant_vals, quant_sve_i);
                for (size_t k = 0; k < processed; ++k) {
                    if (quant_vals[k] == -32768) {
                        quantizer.force_save_unpred(ori[k]);
                    }
            ...
            svst1h_s64(active_pg64, quant_inds + quant_index, quant_sve_i);
            quant_index += processed;
        }
        else if constexpr (CompMode == COMPMODE::DECOMP) {
            const svint64_t quant_sve_i =
                svld1sh_s64(active_pg64, quant_inds + quant_index);
            const svfloat64_t decompressed = svmla_n_f64_x(
                active_pg64, sum, svcvt_f64_s64_x(active_pg64, quant_sve_i),
                real_ebx2);
            if (offset == 1) {
                svst1_f64(active_pg64, data + start, decompressed);
            } else {
                const svuint64_t indices = svmul_n_u64_x(
                    active_pg64, svindex_u64(static_cast<uint64_t>(start), 1),
                    static_cast<uint64_t>(offset));
                svst1_scatter_u64index_f64(active_pg64, data, indices, decompressed);
            }
            const svbool_t esc_pred =
                svcmpeq_n_s64(active_pg64, quant_sve_i, -32768);
            if (svptest_any(active_pg64, esc_pred)) {
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear_and_quantize_1D_line(
        ...
        T pred_edge;
        if (n < 3) {
            pred_edge = data[cur_ij_offset + ((even_len - 1) << 1) * stride];
        } else {
            pred_edge = interp_linear1(data[cur_ij_offset + ((even_len - 2) << 1) * stride],
                                       data[cur_ij_offset + ((even_len - 1) << 1) * stride]);
        }
        int last = 2 * odd_len - 1;
        quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + last * stride, data[cur_ij_offset + last * stride], pred_edge);
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_cubic_and_quantize_1D(const T * buf, const size_t &len, T* data,
       ...
       // assert(len <= max_dim);
        ...
        for (; i + 3  < even_len; ++i) {
            size_t start = ((i << 1) + 3) * offset;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start,  data[start], interp_cubic(buf[i], buf[i + 1], buf[i + 2], buf[i + 3]));
        }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear_and_quantize(const T * a, const T* b, size_t &len, T* data,
        ...
        for (; i < len; ++i) {
            ...
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start,  data[start], interp_linear(a[i], b[i]));
        }
    }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_cubic_and_quantize(const T * a, const T* b, T* c, T*d, size_t &len, T* data,
        ...
        for (; i < len; ++i) {
            size_t start = i * offset;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start,  data[start], interp_cubic(a[i], b[i], c[i], d[i]));
        }
    }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_equal_and_quantize(const T * a, size_t &len, T* data,
        ...
        for (; i < len; ++i) {
            size_t start = i * offset;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start,  data[start], a[i]);
        }
    }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_linear1_and_quantize(const T * a, const T* b, size_t &len, T* data,
        ...
        for (; i < len; ++i) {
            size_t start = i * offset;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start,  data[start], interp_linear1(a[i], b[i]));
        }
    }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_quad1_and_quantize(const T * a, const T* b, const T* c, size_t &len, T* data,
        ...
        for (; i < len; ++i) {
            size_t start = i * offset;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start,  data[start], interp_quad_1(a[i], b[i], c[i]));
        }
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::interp_quad2_and_quantize(const T * a, const T* b, const T* c, size_t &len, T* data,
        ...
        for (; i < len; ++i) {
            size_t start = i * offset;
            quantize_point<CompMode, SkipOverwrite>(cur_ij_offset + start,  data[start], interp_quad_2(a[i], b[i], c[i]));
        }
