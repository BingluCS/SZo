#ifndef SZo_REGRESSION_PREDICTOR_HPP
#define SZo_REGRESSION_PREDICTOR_HPP
...
class RegressionPredictor : public concepts::PredictorInterface<T, N> {
   ...
   private:
    ...
    void pred_and_quantize_coefficients() {
        // bias coeff quant codes by +radius so signed codes become non-negative [0,2*radius),
        // matching HuffmanEncoder<int>'s [0,1<<16) freq table. Both quantizers share the same radius.
        const int radius = quantizer_liner.get_out_range().second / 2;
        for (int i = 0; i < static_cast<int>(N); i++) {
            regression_coeff_quant_inds.push_back(
                quantizer_liner.quantize_and_overwrite(current_coeffs[i], prev_coeffs[i]) + radius);
        }
        regression_coeff_quant_inds.push_back(
            quantizer_independent.quantize_and_overwrite(current_coeffs[N], prev_coeffs[N]) + radius);
    }

    void pred_and_recover_coefficients() {
        const int radius = quantizer_liner.get_out_range().second / 2;   // un-bias: stored code - radius -> signed code
        for (int i = 0; i < static_cast<int>(N); i++) {
            current_coeffs[i] =
                quantizer_liner.recover(current_coeffs[i], regression_coeff_quant_inds[regression_coeff_index++] - radius);
        }
        current_coeffs[N] =
            quantizer_independent.recover(current_coeffs[N], regression_coeff_quant_inds[regression_coeff_index++] - radius);
    }
};
