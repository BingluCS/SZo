        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            if constexpr (EnableInnerOmp) {
                ...
                for (size_t chunk = 0; chunk < vec_chunks; ++chunk) {
                    ...
                    size_t start = cur_i;
                    quantize_float<CompMode, step>(sum, start, odd_data, odd_offset, odd_len, cur_tid);
                }
                i = vec_chunks * step;
            } else {
                for (; i + step < odd_len; i += step) {
                    ...
                    __m256 sum = _mm256_mul_ps(_mm256_add_ps(_mm256_loadu_ps(a), _mm256_loadu_ps(b)), factor);
                    size_t start = i;
                    quantize_float<CompMode, step>(sum, start, odd_data, odd_offset, odd_len, tid);
                }
            }
            ...
            if constexpr (EnableInnerOmp) {
                ...
                for (size_t chunk = 0; chunk < vec_chunks; ++chunk) {
                    ...
                    size_t start = cur_i;
                    quantize_double<CompMode, step>(sum, start, odd_data, odd_offset, odd_len, cur_tid);
                }
                i = vec_chunks * step;
            } else {
                for (; i + step < odd_len; i += step) {
                    ...
                    __m256d sum = _mm256_mul_pd(_mm256_add_pd(_mm256_loadu_pd(a), _mm256_loadu_pd(b)), factor);
                    size_t start = i;
                    quantize_double<CompMode, step>(sum, start, odd_data, odd_offset, odd_len, tid);
                }
            }
        }
        for (; i + 1 < odd_len; ++i) {
