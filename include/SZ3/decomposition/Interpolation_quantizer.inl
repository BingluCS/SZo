...
#ifndef SZ3_QUANT16_HELPERS
#define SZ3_QUANT16_HELPERS
    ...
    ALWAYS_INLINE void InterpolationDecomposition<Tuning, T, N, Quantizer>::quantize_float (__m256& sum, size_t& start, T*& data, size_t& offset, size_t& len) {
        if constexpr (CompMode == COMPMODE::COMP) {
            ...
            if (offset == 1 && full_vector) {
                ...
                ori[0] = data[(start) * offset];
                ori[1] = data[(start + 1) * offset];
                ori[2] = data[(start + 2) * offset];
                ori[3] = data[(start + 3) * offset];
                ori[4] = data[(start + 4) * offset];
                ori[5] = data[(start + 5) * offset];
                ori[6] = data[(start + 6) * offset];
                ori[7] = data[(start + 7) * offset];
            ...
            if (offset == 1 && full_vector) {
                ...
                while (esc) {
                    int k = __builtin_ctz(esc);
                    data[start + k] = quantizer.recover_unpred();
                    esc &= esc - 1;
                }
            } else {
                float tmp[8];
